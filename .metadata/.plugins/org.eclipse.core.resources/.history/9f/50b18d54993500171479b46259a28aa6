...
public class GestorBDSQLite {
	// private static final String RUTA_BD="tpv.sqlite";
	private static final String RUTA_BD = "bbdd/Bar.sqlite";
	// private static final String CREATE_TABLE="CREATE TABLE 'tareas' ('ID'
	// INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL , 'Titulo' TEXT, 'Hecho'
	// INTEGER);";
	...
	public GestorBDSQLite() {
		...
		try {
			...
			if (f.exists()) {
				...
				cn = DriverManager.getConnection("jdbc:sqlite:" + RUTA_BD);
				st = cn.createStatement();
				// st.executeUpdate(CREATE_TABLE); //??? crea bd en la
				// carpetaPersonal
			}
	...
	public void updateSQL(String sql) {
		...
		try {
			...
			if (e.getMessage().startsWith("UNIQUE constraint failed:")) {
				JOptionPane.showMessageDialog(null, "El elemento ya existe...", "Error!", 0, null);
			} else {
				e.printStackTrace();
			}
		}
